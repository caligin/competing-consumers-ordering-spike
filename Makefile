...
query-for-nonbroken-mongo:
	docker exec spike-mongo mongo things --eval  'db.things.find({state: {$$ne: "borked"}}).forEach(printjson)'
	docker exec spike-mongo mongo things --eval  'db.things.find({state: {$$ne: "borked"}}).count()'
